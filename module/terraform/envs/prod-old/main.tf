...
## -------------------------------------------------------------#
/*
module "ec2_abnormal_stop" {
  source = "../../modules/monitoring/ec2-abnormal-stop"
...
module "health_dashboard" {
  source = "../../modules/monitoring/health-dashboard"
  ...
  sns_topic_slack_arn = module.slack.sns_topic_slack_arn
  eventbridge_rule_sns_target_role_arn = module.slack.eventbridge_rule_sns_target_role_arn
  # heath dashboardで監視するサービスリスト(空リストの場合は全サービス)
  health_dashboard_services = []
