...
resource "aws_cloudwatch_log_group" "ecs_flask_abnormal_stop" {
  name = "/aws/events/${local.PJPrefix}-${local.EnvPrefix}-ecs-test-abnormal-stop"
...
resource "aws_cloudwatch_event_rule" "ecs_flask_abnormal_stop" {
  name                = "${local.PJPrefix}-${local.EnvPrefix}-ecs-test-abnormal-stop"
  event_pattern = <<-EOT
  {
    "source": [
      "aws.ecs"
...
resource "aws_cloudwatch_event_target" "ecs_flask_abnormal_stop_log" {
  ...
  rule = "${local.PJPrefix}-${local.EnvPrefix}-ecs-test-abnormal-stop"
