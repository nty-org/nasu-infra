...
resource "aws_codedeploy_deployment_group" "flask_blue_green" {
  app_name               = aws_codedeploy_app.flask.name
  deployment_group_name  = "ECS-BlueGreen-flask-dg"
  ...
  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [aws_lb_listener.flask_http.arn]
      }

      target_group {
        name = aws_lb_target_group.flask["blue"].name
      }

      target_group {
        name = aws_lb_target_group.flask["green"].name
      }
    }
  }
}
